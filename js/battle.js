...
function _B(e,t){for(const i of t){const s=i.effects.filter(r=>r.op==="cooldown_reduce");if(s.length===0)continue;const a=os(s,r=>_h(r));if(e.skills.some(r=>e.cooldowns[r.id]===a))return i.action}return null};
function Ze(e){return dp(e)/4294967296};
function OB(e,t){const i=DI(e),s=t?.statusEffects.find(a=>a.type==="def_pierce");return s?i*Math.max(0,1-s.flat/100):i};
function $B(e,t,i,s,a,g){const r=e.hp/e.maxHp,u=[];for(const n of t.units)n!==e&&zy(n,r)&&(n.mp=0,n.statusEffects=[],u.push({unitId:n.id,delta:n.hp,newHp:n.hp}));return u.length===0?!1:(s.push({type:"skill",turn:a,actorId:e.id,skillId:i.id,text:`${e.name}의 ${i.name}! (HP ${Math.round(r*100)}%)`,hpChanges:u,mpChanges:[],cdChanges:[],snapshots:_e(g)}),!0)};
function u9(e){const t=e.flat??0,i=e.percent??0,s=e.coefficient??0,a=[];return s>0&&a.push(` ATK×${s}`),e.status==="dmg_cap"?t>0&&a.push(` ${Math.round(t)}`):t>0?a.push(` +${Math.round(t)}`):t<0&&a.push(` ${Math.round(t)}`),i>0?a.push(` +${Math.round(i)}%`):i<0&&a.push(` ${Math.round(i)}%`),a.join("")};
function g9(e,t){let i=1/0;for(const s of t.statusEffects)s.type==="dmg_cap"&&s.flat<i&&(i=s.flat);return Math.min(e,i)};
function ql(e){return e.units.every(Fy)};
    ...
    switch(c.op){
      case"hp":{
        ...
        for(const b of f){
          ...
          if(x>0){
            ...
            const w=applyDamageMultiplier(Math.abs(x)*a,b);
      ...
      case"atk_damage":{
        ...
        for(const b of f){
          ...
          p.push({unitId:b.id,delta:-w,newHp:b.hp});
          if(t.hp>0){
            const C=b.statusEffects.find(E=>E.type==="damage_reflect"&&E.turnsLeft>0),P=C&&w>0?w*C.flat/100:0,E=applyDamageMultiplier(P,t);
            if(E>0){t.hp=Math.max(0,t.hp-E);p.push({unitId:t.id,delta:-E,newHp:t.hp});S.push(`반사 ${Math.round(E)}`)}
          }
          S.push(y?`${Math.round(w)}`:`${b.name}(${Math.round(w)})`);
        }
        if(c.target==="enemy_one"&&f.length===1){
          ...
          if(w>0){const C=n?i:s;for(const P of C.units)if(P.hp>0&&!f.includes(P)){const E=applyDamageMultiplier(w,P);P.hp=Math.max(0,P.hp-E);p.push({unitId:P.id,delta:-E,newHp:P.hp})}}
        }
        y&&S.length>0?d.push(`${y}(${S.join(", ")})`):d.push(...S);
        break;
      }
...
function $y(e,t){const i=e.filter(s=>s.hp>0&&s.statusEffects.some(Dh));if(i.length!==0)return i.reduce((s,a)=>os(a.statusEffects.filter(Dh),g=>xv(g,a))>os(s.statusEffects.filter(Dh),g=>xv(g,s))?a:s)};
...
function xv(e,t){return Math.max(0,e.turnsLeft)*rawStatusDamage(e,t)};
...
function HI(e,t){for(const i of e){if(i.op!=="dispel"&&i.op!=="dispel_all"||i.target!=="enemy_one")continue;const s=t.find(a=>a.hp>0&&Ny(i,a));if(s)return s}};
function WB(e,t,i,s){const a=t.units.filter(n=>n.hp>0),g=i.units.filter(n=>n.hp>0);if(g.length===0)return null;const r=Vi(e),u=[];for(const n of UI(e))n.type!=="resurrect_aoe"&&n.type!=="resurrect_one"&&u.push({effects:WI(n,r),action:{kind:"skill",skill:n}});return u.length===0?null:NI(e,u,a,g,s)};
function ky(e){return`${e}${EB(PB(e))?"이":"가"}`};
...
function ao(e,t,i){const s=OB(t,i);return Math.max(1,e*mb/(mb+s))};
function Ny(e,t){return e.op==="dispel_all"?t.statusEffects.length>0:t.statusEffects.some(KI)};
...
function zB(e,t,i,s,a,g){
  ...
  if(u){
    const p=u.statusEffects.filter(Dh),h=applyDamageMultiplier(os(p,d=>xv(d,u))/2*(e.enhancement+1),u);
...
function rB(e){const t=e.endsWith("~t"),i=t?e.slice(0,-2):e,s=i.indexOf("("),a=lB(i,s);if(s>=0&&(a<0||a!==i.length-1))return null;const g=s>=0?i.slice(0,s):i,r=s>=0?i.slice(s+1,a):null,u=g.lastIndexOf("+");if(u<0)return null;const n=g.slice(0,u);if(!mo(n))return null;const o=Number(g.slice(u+1));if(!Number.isInteger(o)||o<0)return null;if(r===null)return{itemCode:n,enhancement:o,...t&&{untradable:!0}};const p=aB(r),l=[];for(const h of p)if(h==="")l.push(null);else{const d=et(h);if(!d)return null;l.push(d)}return{itemCode:n,enhancement:o,engraved:l,...t&&{untradable:!0}}};
...
function NB(e,t,i,s,a,g){
  ...
  if(u){
    const p=applyDamageMultiplier(250*(e.enhancement+1),u);
    if(consumePotionGuard(u))o=`${t.name}: ${r} 사용! ${u.name}(장막)`;
...
function lB(e,t){if(t<0)return-1;let i=0;for(let s=t;s<e.length;s++)if(e[s]==="(")i++;else if(e[s]===")"&&(i--,i===0))return s;return-1};
function n9(e,t,i,s){let a,g;for(const r of e){if(r.op==="dispel"||r.op==="dispel_all"){r.target==="enemy_one"&&(g=i.find(u=>u.hp>0&&u.statusEffects.length>0));continue}r.op==="status"&&(r.target==="ally_one"?a=i9(r.status,t):r.target==="enemy_one"&&(g=s9(r,i,s)))}for(const r of e)Lb(r)&&r.target==="enemy_one"&&(g=Vv(i));return{allyTarget:a,enemyTarget:g}};
function d9(e){const t=Vi(e),i=e.skills.filter(s=>s.type==="attack_aoe");return i.length===0?0:Math.max(...i.map(s=>t*s.coefficient))};
function aB(e){if(e==="")return[""];const t=[];let i=0,s=0;for(let a=0;a<e.length;a++)e[a]==="("?i++:e[a]===")"?i--:e[a]===","&&i===0&&(t.push(e.slice(s,a)),s=a+1);return t.push(e.slice(s)),t};
function i9(e,t){const i=t.filter(s=>s.hp>0&&!s.statusEffects.some(a=>a.type===e));if(i.length!==0)switch(e){case"atk_buff":return i.reduce((s,a)=>a.baseAtk>s.baseAtk?a:s);case"def_buff":return i.reduce((s,a)=>a.baseDef>s.baseDef?a:s);default:return i[0]}};
...
export function buildAlly(advList, gd, raid=false){
  const seen = new Set();
  return advList.filter(a=>a&&a.id).map((a,idx)=>{ const adv=gd.adventurers[a.id]; if(!adv) throw new Error("모험가 없음: "+a.id);
    if(seen.has(a.id)) throw new Error("같은 모험가는 한 번만 편성할 수 있습니다: "+adv.name);
    seen.add(a.id);
    const eq=a.equip?scaleEquip(gd.equipment_stats[a.equip], a.equipEnh||0):null;
    return { id:`${a.id}_${idx}`, name:adv.name, spriteKey:adv.spriteKey, rawAtk:adv.atk, rawDef:adv.def,
      ...
      ...(raid?{damageTakenMultiplier:onyxDamageMultiplier(a.engraved)}:{}) }; });
}
...
function buildPotions(potList, gd){
  return (potList||[]).map(p=>{ const pc=gd.potion_combat?.[p.code]; if(!pc) return null; const enh=Math.max(0,Math.min(40,p.enh||0));
    return { itemCode:p.code, enhancement:enh, effects:(pc.effects[enh]||[]).map(o=>({...o})) }; }).filter(Boolean);
}
export function simulate(party, zoneId, gd, seed){
  it=gd.items||{};
...
function buildRaidSide(party, gd, prefix){
  const units=buildAlly(party?.adventurers||[], gd, true).map(unit=>({...unit,id:`${prefix}${unit.id}`}));
  return {
    units,
